...
class GeneticAlg:
    ...
    def mutate_population(self, population, courses, dependencies, profs, hard_prefs, soft_prefs, course_prefs, rooms):
        for i in range(POPULATION_SIZE):
            self.mutate_schedule(population.get_schedules()[i], courses, dependencies, profs, hard_prefs, soft_prefs, course_prefs, rooms)
        return population
    ...
    def run(self, courses, dependencies, profs, hard_prefs, soft_prefs, course_prefs, rooms):
        #pop will take in data and make scheds
        population = Population(POPULATION_SIZE, courses, dependencies, profs, hard_prefs, soft_prefs, course_prefs, rooms)
        ...
        while population.get_schedules()[0].get_fitness() != 1.0 and generation < MAX_ITERATIONS:
            generation += 1
            #pass in data
            population = self.evolve_population(population, courses, dependencies, profs, hard_prefs, soft_prefs, course_prefs, rooms)
            print("4")
            population.get_schedules().sort(key=lambda x: x.get_fitness(), reverse=True)

            print("generation: " + str(generation) + ", fitness:", population.get_schedules()[0].get_fitness())
            #print(population.get_schedules()[0].hard_softConflicts())
            print(population.get_schedules()[0])
